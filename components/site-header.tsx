...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-[#D5E1E7] bg-white/95 backdrop-blur">
      <div className="mx-auto flex min-h-[100px] max-w-[1500px] items-center justify-between gap-4 px-4 sm:min-h-[114px] sm:px-6 lg:min-h-[124px] lg:px-8">
        <BrandMark />
        ...
        <Link className="hidden min-h-12 items-center rounded-xl bg-navy px-4 py-2 text-sm font-extrabold text-white transition-colors hover:bg-[#214562] xl:inline-flex" href="/request-assessment">Request an Assessment</Link>
        <button ref={toggleRef} aria-controls="mobile-menu" aria-expanded={menuOpen} aria-label={`${menuOpen ? "Close" : "Open"} navigation menu`} className="grid h-12 w-12 shrink-0 place-items-center rounded-xl border border-navy/15 bg-white text-navy transition-colors hover:bg-soft xl:hidden" onClick={() => setMenuOpen((open) => !open)} type="button"><span aria-hidden="true" className="grid gap-1.5"><span className={`block h-0.5 w-5 rounded-full bg-current transition ${menuOpen ? "translate-y-2 rotate-45" : ""}`} /><span className={`block h-0.5 w-5 rounded-full bg-current transition ${menuOpen ? "opacity-0" : ""}`} /><span className={`block h-0.5 w-5 rounded-full bg-current transition ${menuOpen ? "-translate-y-2 -rotate-45" : ""}`} /></span></button>
      </div>
      <div ref={menuRef} className={`${menuOpen ? "block" : "hidden"} max-h-[calc(100vh-100px)] overflow-y-auto border-t border-navy/10 bg-white px-4 py-4 xl:hidden`} id="mobile-menu">
