        ...
        <nav aria-label="Primary navigation" className="hidden items-center gap-0.5 text-[15px] font-semibold xl:flex 2xl:gap-1">
          {desktopNavItems.map((item) => item.href === "/services" ? (
            <div className="relative" key={item.href} ref={servicesRef}>
              ...
              {servicesOpen ? <div className="nav-dropdown absolute left-1/2 top-[calc(100%+.55rem)] w-[390px] -translate-x-1/2 rounded-[18px] border border-[#C9D8E0] bg-white p-2.5 shadow-[0_20px_52px_rgba(23,50,77,.14)]"><div className="border-b border-[#DCE6EA] pb-2"><Link aria-current={pathname === "/services" ? "page" : undefined} className="dropdown-link block rounded-xl px-4 py-3 font-semibold text-navy" href="/services">Services Overview</Link></div><div className="grid gap-1 pt-2">{primaryServices.map((service)=><Link aria-current={isActive(pathname,servicePath(service)) ? "page" : undefined} className="dropdown-link rounded-xl px-4 py-3 text-[0.925rem] font-semibold leading-5 text-navy" href={servicePath(service)} key={service.slug}>{service.navigationLabel}</Link>)}</div></div> : null}
            </div>
      ...
      <div ref={menuRef} className={`${menuOpen ? "mobile-menu-panel block" : "hidden"} max-h-[calc(100vh-76px)] overflow-y-auto border-t border-navy/10 bg-[#FAFCFC] px-4 py-4 shadow-[0_18px_40px_rgba(23,50,77,.09)] sm:max-h-[calc(100vh-80px)] xl:hidden`} id="mobile-menu">
        <nav aria-label="Mobile navigation" className="mx-auto max-w-7xl text-navy">
          <div className="divide-y divide-[#E1EAEE] overflow-hidden rounded-[16px] border border-[#D5E1E7] bg-white shadow-[0_8px_24px_rgba(23,50,77,.045)]">
            ...
            <details><summary className={`mobile-nav-link cursor-pointer pr-12 ${isActive(pathname,"/services") ? "text-teal" : ""}`}>Services</summary><div className="mobile-nav-submenu ml-4 border-l border-[#CCD9DF] bg-[#F7FAFB] py-1.5"><Link aria-current={pathname === "/services" ? "page" : undefined} className="mobile-nav-sublink" href="/services">Services Overview</Link>{primaryServices.map((service)=><Link aria-current={isActive(pathname,servicePath(service)) ? "page" : undefined} className="mobile-nav-sublink" href={servicePath(service)} key={service.slug}>{service.navigationLabel}</Link>)}</div></details>
