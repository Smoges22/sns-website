...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-navy/10 bg-white/92 shadow-[0_1px_0_rgba(23,50,77,0.04)] backdrop-blur-xl">
      <div className="mx-auto flex min-h-[76px] max-w-7xl items-center justify-between gap-5 px-5 sm:px-6 lg:px-8">
        <BrandMark />
        <nav aria-label="Primary navigation" className="hidden items-center gap-1 rounded-full border border-navy/10 bg-soft/80 p-1 text-sm font-bold text-slate lg:flex">
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={`rounded-full px-4 py-2 transition ${active ? "bg-white text-navy shadow-sm" : "hover:bg-white hover:text-navy"}`}
                href={item.href}
                key={item.href}
              >
                {item.label}
        ...
        <div className="hidden items-center gap-3 lg:flex">
          <PortalLink className="text-sm font-extrabold text-navy transition hover:text-teal" />
          <Link className="inline-flex min-h-11 items-center rounded-lg bg-navy px-4 py-2 text-sm font-extrabold text-white transition hover:bg-[#214562]" href="/request-assessment">
      ...
      <div className={`${menuOpen ? "block" : "hidden"} border-t border-navy/10 bg-white lg:hidden`} id="mobile-menu">
        <nav aria-label="Mobile navigation" className="mx-auto grid max-w-7xl gap-1 px-5 py-4 text-sm font-bold text-navy sm:px-6">
          ...
          <PortalLink className="rounded-lg px-3 py-3 hover:bg-soft" comingSoonClassName="rounded-lg px-3 py-3 text-slate" />
